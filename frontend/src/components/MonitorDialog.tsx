...
export function MonitorDialog({
  ...
  const showComponentSetup = form.extract_mode === "component";

  return (
    <dialog
      id="monitor-setup-dialog"
      ref={dialogRef}
      className={showComponentSetup ? "monitor-setup-dialog-wide" : undefined}
      onClose={onClose}
      onCancel={onClose}
    >
      <form onSubmit={(e) => void handleSubmit(e)}>
        ...
        <fieldset className="dialog-fields" disabled={loading || saving}>
          ...
          <div className="row">
            <label>
              ...
              <select
                ...
                onChange={(e) =>
                  update("extract_mode", e.target.value as MonitorFormData["extract_mode"])
                }
              >
          ...
          <label>
            ...
            <input
              required
              placeholder={showComponentSetup ? "点选上方候选组件，或手动填写" : "#content"}
              value={form.selector}
